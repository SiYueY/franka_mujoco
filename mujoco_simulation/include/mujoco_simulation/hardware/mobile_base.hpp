...
class MobileBase : public HardwareInterface<MobileBaseData, MobileBaseCommand, MobileBaseState> {
 public:
  explicit MobileBase(const std::vector<Joint*>& traction_joints);
  ~MobileBase() override = default;

  bool init(const MobileBaseData& data) override;
  bool reset() override;
  bool write(const MobileBaseCommand& command) override;
  bool read(MobileBaseState& state) override;

  const MobileBaseData& data() const { return data_; }
