...
bool Camera::init(const CameraData& data) {
  ...
  if (model_ == nullptr || mj_data_ == nullptr || scene_ == nullptr || render_context_ == nullptr) {
    last_error_ = "MuJoCo rendering state is not available for camera '" + data.name + "'.";
    return false;
  }

  camera_id_ = mj_name2id(model_, mjOBJ_CAMERA, data.camera_name.c_str());
  if (camera_id_ < 0) {
...
bool Camera::read(CameraState& state) {
  ...
  mjv_updateScene(model_, mj_data_, &mj_option_, nullptr, &mj_camera_, mjCAT_ALL, scene_);
  mjr_render(viewport_, scene_, render_context_);
  ...
  mjr_readPixels(rgb_ptr, depth_ptr, viewport_, render_context_);
